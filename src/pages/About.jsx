...
export default function About() {
  ...
  return (
    <div className={isDarkMode ? "bg-gray-900 text-white min-h-screen py-12" : "bg-white text-[#050272] min-h-screen py-12"}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-12">
          ...
          <div className={`absolute inset-0 ${isDarkMode ? "bg-gray-800 opacity-30" : "bg-gray-100 opacity-30"}`}></div>
          <div className="relative z-10 p-6">
            <motion.p
              className={`text-lg leading-relaxed ${isDarkMode ? "text-gray-300" : "text-[#050272]"}`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        <motion.div
          className={`${isDarkMode ? "mt-12 bg-gray-800 p-8 rounded-lg shadow-lg" : "mt-12 bg-gray-300 p-8 rounded-lg shadow-lg"}`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 1, delay: 1.5 }}
        >
          <h2 className={`text-3xl font-extrabold text-center mb-6 ${isDarkMode ? "text-white" : "text-[#050272]"}`}>
            Наши специалисты
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8"> {/* Изменено на 4 колонки для более широкой сетки */}
            {[{ name: "Мэри", role: "Топ Барбер", img: barber1 },
              ...
              >
                ...
                <h3 className={`text-xl font-semibold mb-2 ${isDarkMode ? "text-white" : "text-[#050272]"}`}>{master.name}</h3>
                <p className={`mb-4 ${isDarkMode ? "text-gray-400" : "text-gray-800"}`}>{master.role}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {/* Блок услуг */}
        <motion.div
          className={`mt-12 p-8 rounded-lg shadow-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-300"}`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 1, delay: 1.5 }}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[{ title: "Стрижки", description: "Стрижки любой сложности." },
              ...
              <motion.div
                key={index}
                className={`p-6 rounded-lg text-center ${isDarkMode ? "bg-gray-700" : "bg-white"}`}
                whileHover={{ scale: 1.05 }}  // Увеличение карточки при наведении
                transition={{ type: "spring", stiffness: 300 }}
              >
                <h3 className={`text-xl font-semibold mb-2 ${isDarkMode ? "text-white" : "text-[#050272]"}`}>{service.title}</h3>
                <p className={`mb-4 ${isDarkMode ? "text-gray-400" : "text-gray-800"}`}>{service.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
